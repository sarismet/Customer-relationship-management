...
const resultToText = {
  [QuestionTypes.CHECKBOXES]: (question, result) => {
    return question.options.filter((option,index) => option._id===result[index]).map(option => option.content).join(", ");
  },
      ...
      .map(question => {
        ...
        filtedResults.forEach(result => {
          ...
          if(question.type ==="CHECKBOXES"){
            ...
            let answers = question.options.map((option) => {
              return option._id;
            }); 
            questionAnswer = answers.filter((answer)=>{
    
              let stringID = ""+question._id+"["+answer+"]";
    
              return result.result[stringID]==="true";
              
            });
    
            
          }else{
            questionAnswer = result.result[question._id];
          }
          
          if (typeof questionAnswer === 'string') {
            optionMap[questionAnswer].count++;
          } else if (typeof questionAnswer === 'object') {
            Object.keys(questionAnswer).forEach(index => questionAnswer[index] && optionMap[questionAnswer[index]].count++);
          }
        });
        return {
          _id: id,
  ...
  let textResults = results.map((result, index) => {
    ...
    survey.questions.forEach(question => {
      ...
      if(question.type ==="CHECKBOXES"){
        ...
        questionResult = answers.filter((answer)=>{
        ...
        questionResult = result.result[question._id];
      }
